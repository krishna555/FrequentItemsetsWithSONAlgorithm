...
object task1 {
    def getBidAggrByUid(inputDataRdd: RDD[(String, Long)]) = {
    ...
    def processInput(input_file_path: String, sc: SparkContext, support: Int, case_no: Int, outputPath: String) = {
        val startTime = System.nanoTime()
        val input_rdd = sc.textFile(input_file_path)
        val extracted_data_rdd = input_rdd.zipWithIndex().filter((tupleData) => {
            tupleData._2 > 0
        })
        val allBaskets = if (case_no == 1) getBidAggrByUid(extracted_data_rdd)  else getUidAggrByBid(extracted_data_rdd)
