...
class task2:
    ...
    def process_input(self, input_path, sc, support, filter_threshold, output_file_path):
        ...
        all_baskets = self.get_bid_aggr_by_uid(extracted_data_rdd, filter_threshold)
    ...
    def get_bid_aggr_by_uid(self, input_data_rdd, threshold):

        return input_data_rdd \
            .map(lambda record: self.extract_uid_bid(record)) \
            .groupByKey()\
            .map(lambda aggr_tuple: (aggr_tuple[0], sorted(list(set(list(aggr_tuple[1])))))) \
            .filter(lambda tup: len(tup[1]) > threshold)\
            .map(lambda item_users: item_users[1])
    ...
    def main(self):
        ...
        self.process_input(input_file_path, sc, support, filter_threshold, output_path)
